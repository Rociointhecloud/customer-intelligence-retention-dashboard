...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if seg_path.exists() and tx_path.exists():
        ...
        data_source = f"processed (local) · updated {last_updated(seg_path)}"
    elif demo_seg_path.exists() and demo_tx_path.exists():
        segments = pd.read_csv(demo_seg_path)
        tx = pd.read_csv(demo_tx_path)
        data_source = f"demo sample (cloud) · updated {last_updated(demo_seg_path)}"
    else:
        st.error(
            "No data found.\n\n"
...
def main() -> None:
    ...
    st.caption(
        "Olist e-commerce (2016–2018) · Segmentation + Churn prediction (proxy label)"
    )
    ...
    if pd.notna(min_date) and pd.notna(max_date):
        st.caption(
            ...
            f"({days_span} days) · Data source: {data_source}"
        )
    else:
...
**Goal:** rank customers by risk to prioritize retention actions.
        ...
        st.markdown(
            """
**Limitations**
- Marketplace-heavy dataset: many customers buy once → frequency is very discrete.
